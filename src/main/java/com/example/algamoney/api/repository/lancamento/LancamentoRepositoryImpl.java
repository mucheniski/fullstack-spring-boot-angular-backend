...
public class LancamentoRepositoryImpl implements LancamentoRepositoryQuery {
	...
	public Page<Lancamento> filtrar(LancamentoFilter lancamentoFilter, Pageable pageable) {
		...
		return new PageImpl<>(query.getResultList(), pageable, totalPaginacao(lancamentoFilter));
	}
	...
	private void adicionarRestricoesDePaginacao(TypedQuery<?> query, Pageable pageable) {
		...
		int primeitoRegistroPagina = paginaAtual * totalRegistrosPorPagina;
		query.setFirstResult(primeitoRegistroPagina);
	...
	public List<LancamentoEstatisticaCategoria> porCategoria(LocalDate mesReferencia) {
		...
		criteriaQuery.where(criteriaBuilder.greaterThanOrEqualTo(root.get(Lancamento_.dataVencimento), primeiroDia), criteriaBuilder.lessThanOrEqualTo(root.get(Lancamento_.dataVencimento), ultimoDia));
	...
	public List<LancamentoEstatisticaDia> porDia(LocalDate mesReferencia) {
		...
		criteriaQuery.select(criteriaBuilder.construct(LancamentoEstatisticaDia.class, root.get(Lancamento_.tipo), root.get(Lancamento_.dataVencimento), criteriaBuilder.sum(root.get(Lancamento_.valor))));		
		criteriaQuery.where(criteriaBuilder.greaterThanOrEqualTo(root.get(Lancamento_.dataVencimento), primeiroDia), criteriaBuilder.lessThanOrEqualTo(root.get(Lancamento_.dataVencimento), ultimoDia));
	...
	public List<LancamentoEstatisticaPessoa> porPessoa(LocalDate inicio, LocalDate fim) {
		...
		criteriaQuery.select(criteriaBuilder.construct(LancamentoEstatisticaPessoa.class, root.get(Lancamento_.tipo), root.get(Lancamento_.pessoa), criteriaBuilder.sum(root.get(Lancamento_.valor))));		
		criteriaQuery.where(criteriaBuilder.greaterThanOrEqualTo(root.get(Lancamento_.dataVencimento), inicio), criteriaBuilder.lessThanOrEqualTo(root.get(Lancamento_.dataVencimento), fim));
